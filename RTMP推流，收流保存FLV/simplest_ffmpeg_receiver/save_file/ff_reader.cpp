    ...
    bool* isKeyframe)
{
    ...
    *readType = 0;
	...
	AVStream *in_stream = ff_fmt_ctx_->streams[reading_pakt_.stream_index];
	int time = reading_pakt_.pts * (((float)in_stream->time_base.num) / ((float)in_stream->time_base.den));
	//视频的截取
	if (time >= 4)
	{
		//return NULL;
...
char* FFReader::GetFrame(int* readType, AVFormatContext* ff_fmt_ctx_, AVPacket reading_pakt_, int* readSize, long long* frameTime, bool* iskeyframe, int videoindex)
{
	...
	if (reading_pakt_.stream_index == 0)
	{
		// 音频数据
		if (reading_pakt_.size > 0 /*&& *//*has_got_keyframe*/)
		{
			long long duration = reading_pakt_.duration * 1000.0 / 16000;
			...
			*readSize = reading_pakt_.size;
			...
			//a_timestamp_ += duration;
			//
			if (reading_pakt_.pts > 0)
			{
				a_timestamp_ = av_rescale(reading_pakt_.pts, 1000, 8000);
			}
			...
			*frameTime = a_timestamp_;
			...
			time_base.num = 1;
			a_timestamp_ = av_rescale_q(reading_pakt_.pts + reading_pakt_.duration,
			time_base/*ff_fmt_ctx_->streams[0]->time_base*/, av_make_q(1, 1000));


			//v_timestamp_ += 100;
			//*frameTime = v_timestamp_ ;
			//v_timestamp_ = a_timestamp_;
		}
	}
	else if (reading_pakt_.stream_index == videoindex)
	{
		AVStream* video_stream = ff_fmt_ctx_->streams[video_stream_index_];
		...
		if (reading_pakt_.size > 0/* && has_got_keyframe*/)
		{
			long long duration = reading_pakt_.duration * 1000.0 / 90000;
			//duration *= 2;
...
bool FFReader::FFFindCodecInfo()
{
	...
	//ff_fmt_ctx_->max_analyze_duration = 2000000;
