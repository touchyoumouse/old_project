...
DSGraph::DSGraph()
{
    ...
    if (FAILED(CoCreateInstance(CLSID_SampleGrabber, NULL, CLSCTX_INPROC, 
        IID_IBaseFilter,(void **)&source_filter_)))
        return;

    if (FAILED(source_filter_->QueryInterface(IID_ISampleGrabber, (void **)&sample_grabber_)))
        return;
    ...
    if (FAILED(sample_grabber_->QueryInterface(IID_IBaseFilter, (void **)&grabber_filter_)))
        return;
...
DSGraph::~DSGraph()
{
    if (source_filter_) source_filter_->Release();
...
bool DSGraph::SetCapDevice(const IID& deviceIID, const CString& comObjID)
{
    ...
    hr = moniker->BindToObject(0,0,IID_IBaseFilter, (void **)&source_filter_);
    if( hr != S_OK )
    {
        return false;
    }
